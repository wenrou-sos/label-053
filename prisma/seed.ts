  ...
  const sessions = await Promise.all([
    ...
    prisma.gameSession.create({
      data: {
        ...
        creatorId: users[3].id,
        title: '璀璨宝石 5月例行局',
        description: '月度璀璨宝石聚会',
        startTime: new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000),
        location: '阳光桌游吧 - 中关村店',
        maxPlayers: 4,
        isPublic: true,
        inviteCode: 'BRIGHT005',
        status: 'completed',
      },
    }),
  ])
